...
# Pkg.add(url = "https://github.com/djmorris7/RandomTimeShifts.jl#dev")
...
function diff_SEIR_coeffs!(A, b, β, lifetimes, λ, phis)
    ...
    coeffs_quadratic, C2 = RandomTimeShifts.diff_quadratic(
        β, lifetimes[2], n, λ, phis, [2, 1, 2]
    )
...
function run_initial_condition_experiment()
    ...
    for (id, Z0) in enumerate(Z0s)
        ...
        t_range, pdf_vals, cdf_vals = compute_time_shift_distribution(
            pars, Z0_bp, num_moments
        )
...
# Outputs
    - Tuple of details for the trial.
"""
...
function run_h_benchmarks()
    ...
    for h in h_test
        ...
        prob = ODEProblem(
            F_fixed_s_ode!, [0.0, 0.0], (0, h), pars, saveat = h, save_start = false
        )
        F_offspring(s) = RandomTimeShifts.F_offspring_ode(s, prob)
        coeffs = RandomTimeShifts.moment_coeffs(moments)

        μ = exp(h * λ1)
        lst_w = RandomTimeShifts.construct_lst(coeffs, μ, F_offspring, L, Z0_bp, λ1, h)
        ...
        b1[h] = @benchmark begin
            for s in $s_vals
                $lst_w(s)
            end
        end
        b2[h] = @benchmark begin
            for w in $w_vals
                $W_cdf_ilst(w)
            end
        end
